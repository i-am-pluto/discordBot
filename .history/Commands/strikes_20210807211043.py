...
class strikes(commands.Cog):
    ...
    async def strike(self,ctx,userName: discord.Member, count:int=1, reason=None):
        ...
        try:
            ...
            channel.send(content = userName.mention,embed=discord.Embed(title=f"{count} Strikes were given to {userName}",description=f"Now the user have {newCount} strikes\n{reason}"))
        except:
            pass
        # print(1)
